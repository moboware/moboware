...
#include "socket/tcp_socket_client.hpp"
...
int main(int, char **)
{
  ...
  using TcpClient_t = tcp_socket::TcpSocketClient<SessionHandler>;

  SessionHandler sessionHandler;
  TcpClient_t tcpClient(service, sessionHandler);

  const auto address{std::string{"127.0.0.1"}};
  const auto port{6543u};
  if (tcpClient.Start(address, port)) {
    ...
    const auto timerFn{[&](common::Timer &timer)   //
                       {
                         ...
                         if (tcpClient.SendSocketData({buffer})) {
                           timer.Restart();
                         }
                       }};
