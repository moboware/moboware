...
static std::string_view dh_pem{"-----BEGIN X9.42 DH PARAMETERS-----\n"
...
static std::string_view server_pem{"-----BEGIN CERTIFICATE-----\n"
  ...
  void do_read()
  {
    ...
    socket_.async_read_some(boost::asio::buffer(data_),
                            [this, self](const boost::system::error_code &ec, std::size_t length) {
                              if (!ec) {
                                std::cout << "Read data and sending to back..." << std::endl;
                                do_write(length);
                              }
                            });
  }
...
public:
      ...
      , context_(boost::asio::ssl::context::tlsv13)
  {
    ...
    context_.use_certificate_chain({server_pem.data(), server_pem.size()});
    // context_.use_private_key_file("server.pem", boost::asio::ssl::context::pem);
    context_.use_private_key({server_pem.data(), server_pem.size()}, boost::asio::ssl::context::pem);

    context_.use_tmp_dh({dh_pem.data(), dh_pem.size()});
